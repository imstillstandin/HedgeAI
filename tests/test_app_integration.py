...
def test_app_uses_presentation_helpers_for_decisions() -> None:
    ...
    assert '"final_hedge_ratio": decision.final_hedge_ratio' in app_source
    assert '"hedge_amount_foreign": decision.hedge_amount_foreign' in app_source
    assert '"instrument": format_instrument_label(decision.instrument)' in app_source
    assert '"execution_mode": format_execution_mode(decision.execution_mode)' in app_source
    assert '"reason_codes": format_reason_codes(decision.reason_codes)' in app_source
    assert "if decision.tranches:" in app_source
    assert "from fx_radar.presentation import (" in app_source
    assert "generate_summary_text" in app_source
    assert "def generate_summary_text(" not in app_source
...
def test_legacy_scenario_and_risk_outputs_still_exist() -> None:
    ...
    assert app_source.find("market_context = build_market_context()") < app_source.find("render_hedge_decisions(hedge_decisions)")
    assert app_source.find("render_hedge_decisions(hedge_decisions)") < app_source.find('st.subheader("9. What This Means")')
...
def test_legacy_hedge_range_not_primary_recommendation_text() -> None:
    ...
    assert "live market context" in app_source
    assert "primary action layer" in app_source
...
def test_presentation_risk_summary_excludes_hedge_band_recommendations() -> None:
    ...
    assert "hedge policy" not in summary.lower()
